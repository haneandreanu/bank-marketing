...
st.caption(f"Model: GradientBoostingClassifier · F2-Score terbaik: **{best_score:.4f}**")
...
with st.expander("Tentang aplikasi ini"):
    ...
    **Model:** GradientBoostingClassifier dengan SMOTE untuk menangani *imbalanced data*.
...
with tab_single:
    ...
    with col_c:
        ...
        duration    = st.number_input("Durasi Panggilan Terakhir (detik)", min_value=0, value=200)
    ...
    with eco1:
        emp_var_rate = st.number_input(
            "Employment Variation Rate", value=-1.8, step=0.1, format="%.1f",
            help="Tingkat variasi pekerjaan kuartalan"
        )
    with eco2:
        cons_price_idx = st.number_input(
            "Consumer Price Index", value=92.893, step=0.001, format="%.3f",
            help="Indeks harga konsumen bulanan"
        )
    with eco3:
        cons_conf_idx = st.number_input(
            "Consumer Confidence Index", value=-46.2, step=0.1, format="%.1f",
            help="Indeks kepercayaan konsumen bulanan"
        )
    with eco4:
        euribor3m = st.number_input(
            "Euribor 3 Month Rate", value=1.313, step=0.001, format="%.3f",
            help="Suku bunga euribor 3 bulan harian"
        )
    with eco5:
        nr_employed = st.number_input(
            "Number of Employees (ribuan)", value=5099.1, step=0.1, format="%.1f",
